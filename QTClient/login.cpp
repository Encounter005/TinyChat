...
Login::Login(QWidget *parent) : QDialog(parent), ui(new Ui::Login) {
    ...
    connect(ui->forget_label, &ClickedLabel::clicked, this,
            &Login::slot_forget_pwd);
    connect(HttpManager::getInstance().get(),
            &HttpManager::sig_login_mod_finish, this,
            &Login::slot_login_mod_finish);
    connect(this, &Login::sig_connect_tcp, TcpManager::getInstance().get(),
            &TcpManager::slot_tcp_connect);
    connect(TcpManager::getInstance().get(), &TcpManager::sig_con_success, this,
...
void Login::initHttpHandlers() {
    ...
    _handlers.insert(ReqId::ID_LOGIN_USER, [this](QJsonObject jsonObj) {
        ...
        qDebug() << " uid is " << si.Uid << " host is " << si.Host
...
void Login::on_login_button_clicked() {
    ...
    HttpManager::getInstance()->PostHttpReq(
        QUrl(gate_url_prefix + "/user_login"), json_obj, ReqId::ID_LOGIN_USER,
        Modules::LOGINMOD);
}
...
void Login::slot_tcp_connection_finish(bool success) {
    if (success) {
        ...
        TcpManager::getInstance()->sig_send_data(ReqId::ID_CHAT_LOGIN_REQ,
                                                 jsonString);
    } else {
