    ...
    , ui(new Ui::ResetDialog)
{
    ...
    connect(ui->new_pwd_edit, &QLineEdit::editingFinished, this, [this](){
        checkPassValid();
    });
    connect(ui->varify_code_edit, &QLineEdit::editingFinished, this, [this](){
        checkVarifyValid();
    });
    initHttpHandlers();
...
void ResetDialog::initHttpHandlers()
{
    ...
    _handlers.insert(ReqId::ID_GET_VARIFY_CODE, [this](QJsonObject jsonObj){
        ...
        qDebug()<< "receive data is:  " << jsonObj;
    });
    //注册注册用户回包逻辑
...
{
    auto pass = ui->new_pwd_edit->text();
...
{
    auto pass = ui->varify_code_edit->text();
    if(pass.isEmpty()){
...
void ResetDialog::slot_reset_mod_finish(ReqId id, QString res, ErrorCodes error_code)
{
...
void ResetDialog::on_varify_code_btn_clicked()
{
    ...
    qDebug() << "send get_varifycode post";
...
void ResetDialog::on_confirm_btn_clicked()
{
    ...
    json_obj["passwd"] = toMD5(ui->new_pwd_edit->text());
    json_obj["varifycode"] = ui->varify_code_edit->text();
    qDebug() << "send reset_pwd post";
