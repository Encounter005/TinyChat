...
ChatDialog::ChatDialog(QWidget* parent)
    ...
    QPixmap scaledPixmap
        = pixmap.scaled(ui->side_head_label->size(), Qt::KeepAspectRatio);
    ...
    connect(
        ...
        [this](int uid, const QString&) {
            ...
            QPixmap pixmap = AvatarCache::getInstance()->PixmapOrPlaceholder(
                user_info->_uid, user_info->_icon);
            ui->side_head_label->setPixmap(pixmap.scaled(
                ui->side_head_label->size(),
                Qt::KeepAspectRatio,
    ...
    ui->side_chat_label->SetState(
        ...
        "selected_hover",
        "selected_pressed");
    ui->side_contact_label->SetState(
        "normal",
        "hover",
        "pressed",
        "selected_normal",
    ...
    connect(
        ...
        &TcpManager::sig_friend_apply,
        this,
        &ChatDialog::slot_apply_friend);
    // 连接认证添加好友信号
    connect(
        TcpManager::getInstance().get(),
        &TcpManager::sig_add_auth_friend,
        this,
        &ChatDialog::slot_add_auth_friend);
    // 连接自己认证回复信号
    connect(
        TcpManager::getInstance().get(),
        &TcpManager::sig_auth_rsp,
        this,
    ...
    // &ChatDialog::slot_jump_chat_item_from_infopage); 连接聊天列表点击信号
    connect(
        ui->chat_user_list,
    ...
    connect(
        ...
        this,
        &ChatDialog::slot_jump_chat_item_from_page);

    connect(
        TcpManager::getInstance().get(),
        &TcpManager::sig_friend_icon_updated,
        this,
        [](int uid, const QString& iconName) {
            UserManager::getInstance()->UpdateFriendIcon(uid, iconName);

            // 触发下载 + 通知所有组件刷新
            AvatarCache::getInstance()->EnsureDownloaded(uid, iconName);
            emit AvatarCache::getInstance() -> avatarUpdated(uid, iconName);
        });

    // connect(ui->search_edit, &QLineEdit::textChanged, this,
    // &ChatDialog::slot_text_changed); connect(ui->search_edit,
    // &CustomEdit::sig_mouse_clicked, this, [this](){
    ...
    names = std::vector<QString>{
        "llfc", "zack", "golang", "cpp", "java", "nodejs", "python", "rust"};
...
void ChatDialog::addChatUserList() {
    ...
    if (friend_list.empty() == false) {
        ...
        for (auto& friend_ele : friend_list) {
            ...
            // 渲染消息记录
            if (!friend_ele->_chat_msgs.empty()) {
                chat_user_wid->updateLastMsg(friend_ele->_chat_msgs);
            }
...
void ChatDialog::slot_side_contact() {
    ...
    if (_last_widget == nullptr) {
        switchStackedPageAnimated(ui->friend_apply_page);
...
void ChatDialog::slot_apply_friend(std::shared_ptr<AddFriendApply> apply) {
    ...
    UserManager::getInstance()->AddApplyList(
...
void ChatDialog::slot_add_auth_friend(std::shared_ptr<AuthInfo> auth_info) {
    ...
    ui->chat_user_list->setItemWidget(item, chat_user_wid);
    animateListItemIntro(chat_user_wid);
...
void ChatDialog::slot_jump_chat_item_from_page(
...
void ChatDialog::slot_side_setting() {
    ...
    // 设置
    switchStackedPageAnimated(ui->user_info_page);
...
void ChatDialog::slot_friend_info_page(std::shared_ptr<UserInfo> user_info) {
    ...
    switchStackedPageAnimated(ui->friend_info_page);
...
void ChatDialog::loadMoreConUser() {
    ...
    if (friend_list.empty() == false) {
        for (auto& friend_ele : friend_list) {
            ...
            chat_user_wid->SetInfo(
                friend_ele->_uid, friend_ele->_name, friend_ele->_icon);
            QListWidgetItem* item = new QListWidgetItem;
            // qDebug()<<"chat_user_wid sizeHint is " <<
            // chat_user_wid->sizeHint();
            item->setSizeHint(chat_user_wid->sizeHint());
...
void ChatDialog::switchStackedPageAnimated(QWidget* page) {
    if (!page) {
        return;
    }
...
void ChatDialog::animateListItemIntro(QWidget* itemWidget) {
    if (!itemWidget) {
        return;
    }
