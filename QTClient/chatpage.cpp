...
void ChatPage::slot_switch_to_upload() {
    if (!_fileWindow)
        _fileWindow = new FileUploadWindow();
    _fileWindow->setWindowFlags(Qt::CustomizeWindowHint |
                                Qt::FramelessWindowHint);
    _fileWindow->show();
    _fileWindow->activateWindow(); // 激活窗口
    _fileWindow->raise();          // 提升到最前
}
