...
void MessageTextEdit::inputMethodEvent(QInputMethodEvent *event) {
    m_hasActivePreedit = !event->preeditString().isEmpty();
    ...
    for (int index = 0; index < doc.size(); index++) {
        if (doc[index] == QChar::ObjectReplacementCharacter) {
            ...
            while (indexUrl < count) {
                ...
                if (this->document()->toHtml().contains(
                        msg.content, Qt::CaseSensitive)) {
                    indexUrl++;
...
void MessageTextEdit::keyPressEvent(QKeyEvent *e) {
    if ((e->key() == Qt::Key_Enter || e->key() == Qt::Key_Return)
        && !(e->modifiers() & Qt::ShiftModifier) && !m_hasActivePreedit) {
        emit send();
        return;
    }
...
void MessageTextEdit::insertTextFile(const QString &url) {
    ...
    if (fileInfo.size() > 10LL * 1024 * 1024 * 1024) {
        QMessageBox::information(this, "提示", "发送的文件大小不能大于10G");
        return;
    }
...
bool MessageTextEdit::isImage(QString url) {
    QString imageFormat = "bmp,jpg,png,tif,gif,pcx,tga,exif,fpx,svg,psd,cdr,"
...
QPixmap MessageTextEdit::getFileIconPixmap(const QString &url) {
    ...
    // 文件大小
    QRect rectFile(
        50 + 10, textSize.height() + 5, FileSize.width(), FileSize.height());
